...
Product.Config.prototype.configureElement = function(element) 
{
    ...
    if(element.value){
        ...
        //Ajax cart  
        if( prevNextSetting[element.config.id][1] || element.nextSetting){
	    if(prevNextSetting[element.config.id][1])	
            	element.nextSetting = prevNextSetting[element.config.id][1]
    ...
    if ('-1' != pos) {
          ...
          if(confData[parentId]['optionProducts'][key]){ 
             $$('.product-image img').each(function(img){
                  if(img.src == confData[parentId]['optionProducts'][key]['parent_image'] || img.className == ('amconf-parent-'+parentId)){
