...
class Amasty_Conf_Model_Observer 
{
    ...
    {                 
      if (($observer->getBlock() instanceof Mage_Catalog_Block_Product_List) && Mage::getStoreConfig('amconf/general/enable_list')) {
          ...
          foreach ($productsId[1] as $key => $productId){  
              ...
              $blockHtml = Mage::helper('amconf')->getHtmlBlock($_product);
              if($_product->isSaleable()){
                 if($_product->isConfigurable()){
                    ...
                    if($res[0]){
                          $replace =  $res[1][0].$blockHtml.' <div class="actions"><button type="button" title="'.Mage::helper('amconf')->__('Add to Cart').
					'" class="button btn-cart" onclick="'.Mage::helper('amconf')->getClickUrl().
                    '"  amconf="'.Mage::helper('amconf')->getAmconfAttr().'"><span><span>'.Mage::helper('amconf')->__('Add to Cart').'</span></span></button>' ;
