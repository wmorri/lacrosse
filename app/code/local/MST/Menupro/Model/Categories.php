...
class MST_Menupro_Model_Categories extends Mage_Core_Model_Abstract
{
	...
	public function autoShowSubCategory ($categoryId)
	{
		...
		foreach ($categories as $value) {
			...
			$ulClass = $dataHover = '';
			if ( count($hasChild) > 0) {
				$liClass .= "parent dropdown has-submenu";
				$dataHover .= 'data-hover="dropdown"';
				$ulClass .= 'dropdown-menu';
			}
			$this->_tree .= "<li class='" . $liClass . "'>";
	...
	//--------------------------------------------------
	public function autoSub ($categoryId, $tree, $autoShowSub = null, $showMenuInLevel2)
	{
		if ($autoShowSub == null) {
			return false;
		}
		...
		$html = "";
		if($showMenuInLevel2) {
			/**Show autosub of category below header of menu**/
	...
	public function autoSubResponsive ($categoryId, $tree, $autoShowSub = null)
	{
		...
		foreach ($childIds as $childId) {
			...
			if ( count($childIds) > 0) {
				$liClass .= "dropdown";
				$dataHover .= 'class="dropdown-toggle"';
				$ulClass .= 'dropdown-menu';
