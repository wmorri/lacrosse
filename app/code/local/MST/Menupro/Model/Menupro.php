...
class MST_Menupro_Model_Menupro extends Mage_Core_Model_Abstract
{
	...
	function getMenuByGroupId ($group_id, $permission, $storeid) 
	{
		$menu = Mage::getModel('menupro/menupro')->getCollection()->setOrder('position', 'asc');
		...
		$store_fillter_a = array('like'=>'%'. $storeid .'%');
        $store_fillter_b = array('like'=>'%0%');
		$menu->addFieldToFilter('storeids',array($store_fillter_a, $store_fillter_b));
		return $menu;
	}
	public function getAllMenuArr($group_id, $permission, $storeid)
	{
		$menuArr = array();
		$collection = $this->getMenuByGroupId($group_id, $permission, $storeid);
	...
	* @return collection
	*/
 	function getChildMenu ($group_Id, $parent_Id, $permission, $storeid) 
	{
    	$childMenu=Mage::getModel('menupro/menupro')->getCollection()->setOrder('position', 'asc');
		...
		$store_fillter_a = array('like'=>'%'.$storeid.'%');
        $store_fillter_b = array('like'=>'%0%');
		$childMenu->addFieldToFilter('storeids',array($store_fillter_a,$store_fillter_b));
    	$childMenu->addFieldToFilter('status','1');
    	return $childMenu;
    }
	/**
	...
	*/
	...
	public function recursiveMenu ($parentid){
    	...
    	foreach($chils as $value){
    		$this->_grid_nav.="<li id='m-".$value->getMenuId()."' store='".$value->getStoreids()."'>";
    			...
    			$this->_grid_nav.='<a href="#" onclick="MST.editMenu(\''.$edit_link.'\')">'.$value->getTitle().'</a>';
    ...
    public function storeSwitcher()
    {
		...
		$store_switcher="";
		$store_switcher.="<select id='store_switcher' onChange='MST.storeFilter(this.value)'>";
