...
class Fishpig_Wordpress_Model_Resource_Post_Comment extends Fishpig_Wordpress_Model_Resource_Abstract
{
	...
	protected function _beforeSave(Mage_Core_Model_Abstract $object)
	{
		foreach($object->getData() as $key => $value) {
			if (!is_object($value) && !is_array($value)) {
				$object->setData($key, Mage::helper('core')->escapeHtml($value));
			}
		}
		...
		else if (Mage::helper('wordpress')->getWpOption('comment_whitelist')) {
	...
	{
		if (Mage::helper('wordpress')->getWpOption('comments_notify')) {
//			$object->sendNotificationEmail();
		}
		elseif (Mage::helper('wordpress')->getWpOption('comment_whitelist') && $object->getCommentApproved() == '0') {
//			$object->sendNotificationEmail();
		}
		
		if (Mage::helper('wordpress')->getWpOption('moderation_notify') && $object->getCommentApproved() == '0') {
