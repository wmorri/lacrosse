	...
	protected function _applyCaptionShortcode(&$content, $params = array())
	{
		if (($shortcodes =	$this->_getShortcodesByTag($content, 'caption')) !== false) {
			foreach($shortcodes as $shortcode) {
				...
				$template = '<div id="%s" class="wp-caption %s"%s>%s<p class="wp-caption-text">%s</p></div>';
				$style = $params->getAlign() != 'center' ? ' style="width:'.($params->getWidth()+10).'px;"' : '';

				if (preg_match('/(<a[^>]{1,}>.*<\/a>)(.*)$/iU', $shortcode->getInnerContent(), $matches)) {
					$newHtml = sprintf($template, $params->getId(), $params->getAlign(), $style, $matches[1], trim($matches[2]));
				}
				
				$content = str_replace($shortcode->getHtml(), $newHtml, $content);
			}
		}
	}
