...
{
	imgElement = document.getElementsByClassName('belvgcolorswatch-image')[0];
	imgElement.src = "<?php echo $images[0]['big_image'] ?>";
}
...
<?php foreach ($images as $key=>$image) : ?>
	...
	<?php if (Mage::getStoreConfig('smdesign_smdzoom/zoom/enabled') && Mage::getStoreConfig('smdesign_smdzoom/zoom/more_view_change_main_image') && $_product->getData('enable_zoom_plugin') == 1 && ( 1 || $_product->getImage() != 'no_selection' && $_product->getImage() ) ) : ?>
		$(li).update("<a href=\"<?php echo  $image['big_image']; ?>\"  rel=\"<?php echo  $image['image']; ?>\" onclick=\"SMDUpdateMainImage(this);return false;\"><img src=\"<?php echo  $image['thumb'] ?>\" width=\"<?php echo '143'; ?>\" height=\"<?php echo '165'; ?>\" /></a>");
	<?php else : ?>
		$(li).update("<a href=\"#\" onclick=\"popWin('<?php echo Mage::getUrl('catalog/product/gallery', array('id'=>$image['product_id'], 'image'=>$image['id'], 'pid'=>$productId)) ?>', 'gallery', 'width=300,height=300,left=0,top=0,location=no,status=yes,scrollbars=yes,resizable=yes'); return false;\" ><img src=\"<?php echo  $image['big_image'] ?>\" width=\"143\" height=\"165\" /></a>");
